...
בתרגיל זה, עליכם לכתוב פונקציה `CountBeads` שתספור את מספר החרוזים בשרשרת נתונה של אובייקטי `Bead`. הפונקציה תקבל את ראש השרשרת ותחזיר את הספירה הכוללת.
...
במשימה זו, עליכם לממש פונקציה `RemoveBead` שתסיר חרוז ספציפי משרשרת חרוזים מקושרת. השרשרת מיוצגת על ידי אובייקטים מסוג `Bead` המקושרים ביניהם.
