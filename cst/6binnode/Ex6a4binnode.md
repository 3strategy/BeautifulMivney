...
עליכם לממש פונקציה חיצונית בשם `AddLeftChildToLeaves` אשר מקבלת BinNode⟨int⟩ tree המצביע לשורש של עץ בינארי
...
עליכם לממש פונקציה חיצונית בשם `AddSiblingToSingleChild` אשר מקבלת BinNode⟨int⟩ tree המצביע לשורש של עץ בינארי
...
2. עבור רמה L = 1, ההערך שיוחזר יהיה: 2 (הצמתים 5, 15)
3. עבור רמה L = 2, ההערך שיוחזר יהיה: 3 (הצמתים 2, 7, 12)
4. עבור רמה L = 3, ההערך שיוחזר יהיה: 0 (אין צמתים ברמה זו)
...
1. עץ עם מסלול עולה קיים:
    ...
    Console.WriteLine(Solution.HasAscendingPath(root)); // הערך שיוחזר: True
    ```
...
1. עץ ללא מסלול עולה:
    ...
    Console.WriteLine(Solution.HasAscendingPath(root)); // הערך שיוחזר: False
    ```
...
עליכם לממש פונקציה בשם `FindMaxPathSum` שמקבלת BinNode⟨int⟩ tree המצביע לשורש של עץ בינארי ומחזירה מספר שלם (int) המייצג את סכום המסלול המקסימלי מהשורש לעלה כלשהו.

כלומר, אנחנו בוחרים מסלול שמתחיל בצומת שהתקבל כפרמטר, ממשיכים בכל פעם לבן שמאלי או ימני, עד שמגיעים לעלה — כך שסכום הערכים של כל הצמתים במסלול הזה יהיה הגדול ביותר האפשרי.

אם העץ ריק, הפונקציה תחזיר `int.MinValue`.
