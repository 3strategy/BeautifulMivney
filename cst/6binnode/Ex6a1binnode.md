...
## 6a1.4 ספירת צמתים זוגיים {#id6a1.4}
...
## 6a1.5 ספירת צמתים עם שני ילדים זהים {#id6a1.5}

עליכם לממש פונקציה בשם `CountNodesWithTwoIdenticalChildren` אשר מקבלת `BinNode⟨int⟩ tree` המצביע לשורש של עץ בינארי ומחזירה מספר שלם המייצג את מספר הצמתים בעץ שעבורם מתקיים:

1. לצומת יש **שני ילדים** (ילד שמאלי וילד ימני).
2. לשני הילדים יש **ערך זהה**.


### דגשים

* אם `t == null` (עץ ריק) – התשובה היא `0`.
* צומת עם **ילד אחד בלבד** (רק שמאלי או רק ימני) **לא נספר**.
* יש לעבור על כל הצמתים בעץ ולסכום את הכמות הכוללת.

{: .box-success}
**רמז:** פתרון רקורסיבי טיפוסי מורכב מ־

* `me` – האם הצומת הנוכחי נספר (0 או 1)
* ועוד הספירה ברקורסיה לתת־העץ השמאלי והימני.
...
1. עבור העץ הבא:
   ...
   כאן רק הצומת `10` נספר (שני ילדיו קיימים ושווים: `5` ו־`5`).
...
2. עבור העץ הבא:
   ...
   כאן נספרים שני צמתים: `10` (5/5) וגם `30` (40/40).

   פלט: `2`

{: .box-warning}
**שימו לב:** צומת לא נספר אם אחד הילדים חסר, גם אם לילד הקיים יש ערך “נכון”.
