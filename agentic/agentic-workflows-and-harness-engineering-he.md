...
## 2. Prompt to online - asking for agentic prompt

בקשה כזו יכולה להוביל לפרומפט ברור יותר, אך גם לחריגה מהתכנון המקורי. לאחר שמקבלים הצעה - חשוב לעבור עליה ולתקן. זה יחסית דומה ל- flow 5, אבל יכול **להוביל לתבנית לולאה יותר חזקה (אם מבקשים)**. רצוי מאד שתצליחו לבנות לולאה מתישהו. זו חוויה שקשה לתאר. 

אחרי זה, תרצו לשקול מתי זה מתאים ומתי לא. העולם כבר לא עובד על "Token Maxing". התקציב מוגבל.

לפני תחילת מימוש - אני שומר תכניות כאלו כמסמך. דברים משמעותיים עוברים כמה גרסאות טקסט (דיוקים), והתייעצויות. בחירה בין ארכיטקטורות או שינוי ארכיטקטורה שהוצעה, לפני תחילת מימוש. בסוף בתוך הטקסט ה"יפה" אנחנו שולטים ודואגים לקבוע התנהגות וארכיטקטורה. זה שאייגנט התערב במסמך - בא לתת לנו כר רחב יותר למחשבה / חשיפת נקודות שאולי לא חשבנו עליהן. בסוף - עלינו לדייק את *הכל* כדי למנוע rework.

⟵ להרחבה ראו [12 Prompt for a prompt](#id1prompt_for_prompt)
...
```text
There is a bug in x, y, z when a,b,c etc (i.e. very detailed description about the problem so that the agent has a clear idea of how to write the red test - and later to find the root cause. Include **screenshots where applicable**)

Fix it Red-Green-Refactor TDD. (add description about the test if you want (be it unittest, playwright, playwright with screenshots))
Testing level:
T2 during the loop, T3 before completion.
```

<details class="english"><summary>כל זה אני לא רושם לו. הוא יודע לבד מה לעשות עם זה</summary>
...
### זווית Agentic: המפרט הוא חלק מה־harness

ב־SDD השיפור המתמשך אינו תוספת צדדית: מפרט, decision log, task template ו־validation matrix הם artifacts של ה־harness, והם מאפשרים לסוכנים עתידיים לעבוד מאותו מקור אמת (source of truth). בקשו מהסוכן לזהות מה חסר במסמכים הללו, ולא רק מה חסר בקוד. 

אם אתם יודעים מהו - הגדירו לאייג'נט מהו ה-**single source of truth**. זה חשוב במיוחד כאשר מתבררים פערים בין מסמכים שונים, או בינם לבין ההתנהגות בפועל.
...
מדי פעם בקשו מה-Agent לזהות מסמכים שכבר אינם רלוונטים, או לעדכן מסמכים כשהמימוש בפועל מייצג את הדרישה טוב יותר מהמסמך. כל עוד ברור מה ה-source of truth ניתן לציין זאת ולעדכן את מה שצריך (את המסמך או את הקוד)
...
### זווית Agentic: לולאה טובה משדרגת את עצמה דרך מצב מתמשך

כאשר אותה עמימות, חסם או בדיקה איטית חוזרים, הבעיה אינה רק במשימה הבודדת אלא ב־harness של הלולאה. בקשו מהסוכן לשמר את הלמידה בקובץ מצב מתמשך, כמו `TASKS.md`, `SPEC.md` או decision log, כך שהאיטרציה הבאה לא תגלה אותה מחדש. בסופו של דבר גם אם יש מסמכים מתאימים האייג'נט לא יקרא הכל. צריך לדאוג לזקק את המידע ולהנגיש אותו באופן יעיל, כגון AGENTS.md שכולל קטלוג הפניות למסמכים, AGENTS שמפנה לקובצי AGENTS אחרים בתיקיות כו.
...
| # | Risk | Priority | Owner | Resolution note | Date |
|---:|:---|:---|:---|:---|:---|
| 1 | Low | P4 | CODEX | Resolved for the active migrated moon-crate path: item execution now goes through `CrateSubmissionService` -> `CrateExecutionService` -> `PebbleCompilationService`, where negative-normalization uses a syntax-tree rewriter instead of the old string rebuild, and coverage includes nested-expression cases such as `glyphof(...)`. | 2026-01-05 |
| 6 | Low | P4 | All | Resolved on the active lantern-management path: operators can link available z to a group from `/operator/group-z`, control per-group visibility through linked-bundle settings (`display/open/close/deadline`), and remove stale group bundles when needed. | 2026-01-05 |
| 17 | Low | P2 | Roy | "In Progress Only" now includes attempted widgets with a score of 0 by preserving nullable scores and handling null vs. zero distinctly in the filters (`BlueApp/Pages/Index.jsx`, `BlueApp/Components/WidgetList.jsx`). | 2025-10-28 |
{: .table-en}
...
Agent אחד מממש; agent אחר או subagent בוחן את התוצאה מול spec, diff ובדיקות. לאחר מכן המממש מתקן וה־validator מאמת. **לא ניסיתי!**
...
### זווית Agentic: שפרו את מנגנון הבדיקה, לא רק את הקוד

כאשר בדיקה אוטומטית ממציאה קלט שמפיל את המערכת, לא זורקים אותו. זה רמז שמערכת הבדיקות למדה משהו חדש. יכול להיות שחסר כלל שמגדיר מה תמיד צריך להיות נכון (property), שמחולל הקלטים בודק מעט מדי אפשרויות (generator), שלא נשמר המספר שמאפשר לשחזר את אותה ריצה (seed), או שתהליך צמצום הדוגמה הכושלת לא השאיר דוגמה ברורה מספיק (shrinker). בקשו מהסוכן לשמור את הלקח בבדיקות או בתיעוד, כדי שבפעם הבאה הכשל יהיה קל יותר לשחזור ולהבנה.

<div markdown="1" class="box-success">

כשנמצאת דוגמה קטנה שעדיין מפילה את הקוד (counterexample ממוזער), בדקו מה כדאי
לשמור קבוע: בדיקת רגרסיה, כלל בדיקה חזק יותר (property), הרחבה של סוגי הקלטים
שנוצרים אוטומטית (generator), מספר ריצה קבוע לשחזור (seed), או הודעת כשל ולוגים
ברורים יותר (diagnostics). הציעו את השינוי הקטן ביותר שיישאר במערכת, והסבירו
איזה כשל עתידי יהיה קל יותר לראות בזכותו.
...
## 10. Workflow עשירי: לולאת תיקון סגורה (Closed-Loop Repair)

### המונח

**לולאת תיקון סגורה** (Closed-Loop Repair) היא תהליך אוטומטי יחסית:
...
### במה זה שונה מ־Red-Green TDD?

ב־Red-Green TDD אנחנו בדרך כלל מתחילים מבאג או דרישה ברורה, ואז כותבים בדיקה אחת שמוכיחה את הבעיה. המחזור הוא: קודם בדיקה אדומה, אחר כך תיקון מינימלי, ואז ניקוי הקוד.

בלולאת תיקון סגורה (Closed-Loop Repair) נקודת הפתיחה שונה: כבר יש כשל ממערכת קיימת, למשל בדיקות אוטומטיות (CI) שנפלו, לוג שגיאה, בדיקת קצה שנכשלת או דיווח לא ברור. העבודה העיקרית אינה רק "לכתוב בדיקה אדומה", אלא להבין איזה סוג כשל זה, לנסח השערה שאפשר לבדוק, לתקן מעט, להריץ את הבדיקה הצרה ביותר, ולהחליט אם ממשיכים לאיטרציה נוספת או עוצרים.

| שאלה | פיתוח לפי בדיקה אדומה-ירוקה (Red-Green TDD) | לולאת תיקון סגורה (Closed-Loop Repair) |
|---|---|---|
| מאיפה מתחילים? | דרישה או באג שרוצים לתרגם לבדיקה | כשל שכבר הופיע במערכת |
| מה הצעד הראשון? | לכתוב בדיקה שנכשלת מהסיבה הנכונה | לאבחן את הכשל ולנסח השערה ניתנת לבדיקה |
| מה הסכנה המרכזית? | לכתוב בדיקה חלשה מדי או לתקן בלי בדיקה | לשנות קוד אקראית בלי להבין את מקור הכשל |
| מתי משתמשים? | כשרוצים להגן על התנהגות מוגדרת | כשכבר יש משוב מכשל קיים: בדיקות אוטומטיות (CI), לוגים, בדיקות שנפלו או כשל מסביבה דמוית ייצור |
{: .table-he}
...
## 12. Prompt-for-a-Prompt: מתי לבקש ממודל לכתוב את הפרומפט לסוכן? {#id1prompt_for_prompt}
