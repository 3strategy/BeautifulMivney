...
<details markdown="1"><summary>שלוש שכבות שונות פועלות כאן</summary>

1. `Student.java` הוא חוזה המקור שאנחנו עורכים: אילו שדות יש לישות ואילו annotations
   מתארים אותם.
2. בזמן build,‏ Requery מייצרת מחלקות כמו `StudentEntity` ו־`Models`. הן קוד Java, אך הן
   תוצר בנייה — לא עורכים אותן ולא מוסיפים אותן ידנית למדריך.
3. בזמן ריצה, SQLite שומר שורות בקובץ `class-netflix.db`. סגירת Activity או יצירת אובייקט
   Java חדש אינן מוחקות את הקובץ; ניקוי נתוני היישום או הסרה שלו כן מתחילים מסד חדש.

ההבחנה הזאת עוזרת באיתור תקלות: סימון אדום על `StudentEntity` שייך בדרך כלל לשלב ה־build;
נתונים כפולים לאחר restart שייכים ללוגיקת ה־seed; ונתונים שנעלמו אחרי uninstall הם התנהגות
רגילה של אחסון פרטי ליישום.
...
| `binding` ו-`results` | יישארו בכל שלושת הפרקים |
...
[המשך לפרק 2: קשר עם דירוג](/android/sqlite/02.requery-rated-relationship)
...
{: .box-note}
[פתחו את Gem: Requery: מן Entity אל קשר מדורג](https://gemini.google.com/gem/1sscGN-LJ8GuxQRTCMMqfTcdTZ5Dzkqao?usp=sharing) כדי לקבל רמזים, שאלות אבחון והסברים המתאימים לשלב שבו אתם נמצאים.
