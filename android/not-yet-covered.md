...
| אסינכרוניות ותחרות בין פעולות | **מוזכר/משלים; נוספה העמקה עיונית ב־RTDB** | קיימים תרשימי שני מכשירים, הבחנה בין אירוע מקומי לאישור שרת ודיון בדריסת כתיבות; עדיין חסרים תרגול transactions וכשלי רשת, main thread, executor/callback, ביטול ותוצאה שמגיעה אחרי סגירת מסך | [תרשימי זמן אמת](/android/projectSteps/021b.TicTacToeRTDBGame#rtdb-two-device-flow), [השוואת חוזי נתונים וכתיבות](/android/projectSteps/021a.TicTacToeRTDBRooms#presence-rtdb-comparison), [תיאום UI ו־Worker](/android/CollectCircles/17.collect-circles-worker-settlement) |
