...
גרסת WorkManager שבה נשתמש היא `2.11.2`. הוסיפו ל־`gradle/libs.versions.toml`:
...
וב־`app/build.gradle.kts`, בתוך `dependencies`:
...
צרו `PusherEligibilityWorker.java`:
...
בפרק הזה ה־Worker **רק קורא** את כלכלת המשחק ולכן אין פעולת read-modify-write שמתחרה באיסוף. `SharedPreferences` בטוח לקריאה מכמה threads. בפרק הבא, לפני שה־Worker יחשב ויכתוב התקדמות אופליין, נטפל במפורש בתיאום בין הכותבים.
...
בפרק הבא נשתמש באותו Worker בשתי תוכניות: כל שלוש שעות כאשר הסוללה אינה חלשה, וכל 30 דקות כאשר המכשיר בטעינה. WorkManager מבטיח מרווח מינימלי ולא שעת הפעלה מדויקת.
...
{: .box-success}
יצרנו יחידת עבודה אמיתית שאינה תלויה בחיי ה־Activity, בלי להעמיס עדיין תזמון מחזורי. בפרק הבא נוסיף את שני לוחות הזמנים וה־constraints של הסוללה והמטען.

[המשך לפרק 15: עבודה מחזורית עם אילוצי סוללה](/android/CollectCircles/15.collect-circles-periodic-work)
