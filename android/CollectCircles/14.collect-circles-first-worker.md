...
[הפרק הקודם]({{ '/android/CollectCircles/13.collect-circles-offline-progress' | relative_url }}){: data-sequence-nav="prev"} · [הפרק הבא]({{ '/android/CollectCircles/15.collect-circles-periodic-work' | relative_url }}){: data-sequence-nav="next"}
...
בשני מסלולי ההמשך נשלח את אותה בדיקה גם באופן מחזורי. במסלול הקצר של פרק 15b לא נשנה את ה־Worker כלל: נשפר את `canBuyPusher()` כך שתכלול בחישוב גם התקדמות אופליין תיאורטית, ונמשיך להציג את המחיר באמצעות `getNextPusherPrice()`.
...
- [מסלול קצר וסיום — פרק 15b: התראה מחזורית ו־Brag](/android/CollectCircles/15b.collect-circles-notification-only)
- [מסלול העמקה — פרק 15: עבודה מחזורית עם אילוצי סוללה](/android/CollectCircles/15.collect-circles-periodic-work){: data-sequence-nav="next"}
