...
{: .box-warning}
בפרק הזה **אין קוד שרץ ברקע**. אין `Thread`,‏ `Service` או `WorkManager` שפועלים כאשר היישום סגור. הביטוי "התקדמות אופליין" מתאר חישוב שמתרחש רק בפעם הבאה שה־Activity עוברת ל־`onStart`.
...
צרו `OfflineProgressCalculator.java`:
...
1. הפעילו Auto עם Pusher אחד, עברו לרקע לכ־12 שניות וחזרו. אמורים להתווסף בערך שני עיגולים ולהופיע Toast.
2. חזרו על יציאות קצרות. ודאו שהשברים מצטברים ובסופו של דבר הופכים לעיגול שלם.
3. בדקו עם שני Pushers שהקצב בערך כפול.
4. כבו Auto או בדקו עם אפס Pushers; לא צריכה להיות התקדמות אופליין.
5. ודאו שהלוח נבנה מחדש ואינו מציג יותר מ־12 עיגולים.
6. סגרו ופתחו את היישום ובדקו שהיתרה, Lifetime והשבר ממשיכים באופן עקבי.
