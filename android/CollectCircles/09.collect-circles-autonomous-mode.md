...
[הפרק הקודם]({{ '/android/CollectCircles/08.collect-circles-persistent-economy' | relative_url }}){: data-sequence-nav="prev"} · [הפרק הבא]({{ '/android/CollectCircles/10.collect-circles-pusher-shop' | relative_url }}){: data-sequence-nav="next"}
...
### אחרי
    ...
    {% highlight diff mark_lines="6 24" %}
-/**
- * מתחיל סיבוב חדש לפי הגודל הנוכחי של לוח המשחק.
- * אם הלוח עדיין לא נמדד, הפעולה אינה משנה את המשחק.
- */
+/**
+ * מתחיל משחק מתוזמן ועוצר לולאת פריימים אוטונומית קודמת.
+ */
 public void startNewGame() {
...
[המשך לפרק 10: חנות ה־Pushers](/android/CollectCircles/10.collect-circles-pusher-shop){: data-sequence-nav="next"}
