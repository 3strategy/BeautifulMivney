...
[הפרק הקודם]({{ '/android/CollectCircles/11.collect-circles-draw-pusher' | relative_url }}){: data-sequence-nav="prev"} · [הפרק הבא]({{ '/android/CollectCircles/13.collect-circles-offline-progress' | relative_url }}){: data-sequence-nav="next"}
...
ב־`autonomousFrame` של `GameBoardView`, החליפו את הקריאה הקיימת ל־`game.update` בקריאה שמקבלת את הערך המוחזר ומדווחת על כל האיסופים. הלולאה החדשה נכנסת **לפני** `invalidate()`:
...
זהו אותו callback שבו משתמש איסוף ידני. הוא נרשם ב־`MainActivity` באמצעות `binding.gameBoard.setOnCircleCollectedListener(this::recordCollectedCircle)`, ולכן מסלול הקריאה המלא בכל איסוף אוטומטי הוא:
...
ה־`apply()` נמצאת ב־`GameProgress.java`, בתוך `recordCollectedCircle`, ולא בתוך `GameBoardView`. כל הקריאות בשרשרת עד אליה מתבצעות באופן סינכרוני ב־UI thread לפני שלולאת ה־`for` ממשיכה ולפני `invalidate()`. הקריאה ל־`apply()` מעדכנת מיד את עותק ה־`SharedPreferences` שבזיכרון וחוזרת בלי להמתין לכתיבת הקובץ; את כתיבת הדיסק Android משלימה באופן אסינכרוני. כך גם האיסוף הידני וגם האיסוף האוטומטי עוברים באותו מסלול לעדכון Circles ו־Lifetime, בלי לחסום את הפריים על I/O של דיסק.

## 10. בדיקה ממוקדת

החליפו את בדיקת הסיור הישנה `pusherTurnsAroundAndStaysInsideTheBoard` בבדיקה של מחזור העבודה:
...
[המשך לפרק 13: התקדמות בזמן שהיישום סגור](/android/CollectCircles/13.collect-circles-offline-progress){: data-sequence-nav="next"}
