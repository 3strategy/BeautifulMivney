...
[חזרה ל־013: הוספת Activities לתפריט](/android/projectSteps/013addingActivityToMenu){: data-sequence-nav="prev"}

{: .box-note}
בשלב הקודם כל מסך בנה תפריט Overflow משלו. כעת ניצור Activity מארח אחד עם מגירת ניווט,
ובתוכו נוכל להחליף Fragments. כך מעטפת הניווט נשארת קבועה ורק אזור התוכן מתחלף.

<details markdown="1"><summary>Activity, Fragment ו־DrawerLayout — מי אחראי על מה?</summary>

- `MenuActivity` היא חלון Android מלא ובעל מחזור חיים. היא מחזיקה את סרגל הכלים ואת המגירה.
- `DrawerLayout` הוא View שמסדר שכבת תוכן ולצדה מגירה שנפתחת ונסגרת.
- `NavigationView` מציג את פריטי התפריט ושולח אירוע כאשר נבחר פריט.
- `Fragment` הוא חלק מתחלף מן המסך. הוא אינו Activity נוסף ואינו נרשם ב־Manifest.
- `FragmentContainerView` או container בעל `id` מסמן היכן ה־Fragment יציג את ה־View שלו.

ההפרדה הזאת חשובה: פתיחת Activity יוצרת יעד חדש ב־back stack, ואילו החלפת Fragment משנה
אזור בתוך אותה Activity. לכן מחליטים מראש אילו יעדים הם מסכים עצמאיים ואילו הם לשוניות או
חלקים של מעטפת אחת.
...
- [014b - הוספת Home ו־Profile למגירה](/android/projectSteps/014b.AddingFragmentsToMenu){: data-sequence-nav="next"}
