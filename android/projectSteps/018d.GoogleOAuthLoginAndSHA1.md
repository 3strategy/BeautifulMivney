...
[Back to 018c: Email/Password login and FBRef](/android/projectSteps/018c.EmailPasswordLoginAndFBRef){: data-sequence-nav="prev"}
...
- Unlike the image shows, you should ==also have an app called com.example.tictacmenu (or whatever your app package name is)==. it is perfectly possible to have several apps added as part of the config.
- A single app item, named after a package name, ==may have multiple SHA1 fingerprints associated with it== (if several students try sharing the same firebase for the multiplayer experience)
...
Continue using the version catalog introduced in `018c`.

First, edit `gradle/libs.versions.toml` and add the Google Sign-In version under
`[versions]`:
...
Then add the library under `[libraries]`:
...
The catalog key `play-services-auth` becomes the Gradle accessor
`libs.play.services.auth`.

Now edit `app/build.gradle.kts` and add the dependency:
...
NOTE: ==if your project was updated to use bindings - you will not see many of the below elements (Buttons and EditTexts fields declared)==
...
+    private static final int RC_GOOGLE_SIGN_IN = 9001;
     ...
     private CheckBox cBstayconnect; //not required if you use bindinging
     private EditText eTemail;//not required if you use bindinging
     private EditText eTpass;//not required if you use bindinging
     private Button btnLogin;//not required if you use bindinging
+    private Button btnGoogleSignIn;//not required if you use bindinging
     private boolean loginInProgress;//not required if you use bindinging
```

### 4.3 Initialize Google client and button reference
NOTE: ==if your project was updated to use bindings - you no longer have initViews()==
...
{: .box-note}
**Optional design detour:** To give `TicTacMenu` the blue/orange palette, blue drawer toolbar, and clear square game cells, follow [the small theming-alignment tutorial]({{ '/android/projectSteps/020tictacmenu-theme-alignment' | relative_url }}). It changes presentation only and is not required for Google OAuth, SHA1 setup, or any `018d` checkpoint.

## Continue

- [019a - מעבר ל־View Binding](/android/projectSteps/019a.BindingInsteadOfFindByID){: data-sequence-nav="next"}
- [020 - התאמת העיצוב של TicTacMenu](/android/projectSteps/020tictacmenu-theme-alignment)
