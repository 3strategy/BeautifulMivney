...
[חזרה ל־021a: פרסום חדרים ורשימה בזמן אמת](/android/projectSteps/021a.TicTacToeRTDBRooms)
...
3. ה־SDK מפעיל את המאזין המקומי בעקבות הכתיבה, ומסנכרן את השינוי דרך השרת אל שאר המכשירים.
4. בכל מכשיר, המאזין שמקבל snapshot של החדר מנקה את הלוח ובונה אותו מחדש מתחילת המחרוזת.

{: .box-note}
העדכון במכשיר שכתב יכול להופיע לפני שהשרת אישר את הכתיבה. [תרשימי הזרימה בהמשך](#rtdb-two-device-flow) מפרידים בין האירוע המקומי, סנכרון המכשיר השני ואישור הכתיבה.
...
אחרי בדיקת המשבצת אנו בונים מהלך כגון `2,1,O` ומצרפים אותו לרצף. שימו לב שאיננו קוראים כאן ל־`model.setMove` ואיננו משנים את הטקסט של הכפתור. כל ציור הלוח עובר דרך `showSelectedRoom`, בעקבות snapshot שמתקבל במאזין. במכשיר שכתב זה יכול להיות אירוע מקומי של ה־SDK לפני אישור השרת; במכשיר השני השינוי מגיע דרך הסנכרון עם RTDB.
...
## הזרימה השלמה — מה מפעיל איזו מתודה בשני המכשירים?
{: #rtdb-two-device-flow}

<details open markdown="1">
<summary>יצירת חדר, הצטרפות וסבב מהלכים של X ו־O</summary>

התרשימים מתארים את הקוד של **סיום השיעור הזה, 021b**. בכל מכשיר רץ עותק נפרד של `Main2Activity`, עם `selectedRoom`, `model` ומאזינים משלו. העמודות של המכשירים כוללות גם את Firebase SDK המקומי; העמודה האמצעית מייצגת את השרת. חץ שחוזר לאותה עמודה הוא פעולה בתוך אותו מכשיר.

{: .box-success}
X אינו קורא למתודה במכשיר של O. הוא משנה נתון בכתובת המשותפת `/TicTacToeRtdb/games/{roomId}`. ה־SDK במכשיר של O מפעיל את ה־callback שנרשם שם, והקוד של O בונה את המסך שלו מן הנתון שהתקבל.

### א. יצירה והצטרפות — לפני המהלך הראשון

בדוגמה שני המכשירים כבר נמצאים בלובי, `listenForRooms()` כבר חיברה בכל אחד מהם את `gamesListener`, ורק משתמש אחד מנסה להצטרף בתור O.

במסך צר אפשר לגלול את התרשימים הצדה כדי לקרוא את שמות המתודות בגודל נוח.

<div markdown="1" class="rtdb-device-flow">

```mermaid
sequenceDiagram
    autonumber
    participant X as Device X<br/>Activity + SDK
    participant DB as Firebase RTDB<br/>Server
    participant O as Device O<br/>Activity + SDK
    X->>X: startGameAndWait()<br/>push() + new GameRoom(...)
    X->>X: newRoomReference.setValue(room)
    X->>DB: סנכרון החדר החדש
    par המשך היצירה
        DB-->>X: הכתיבה אושרה<br/>addOnSuccessListener
        X->>X: openRoom(roomId)<br/>addValueEventListener(...)
        X->>X: onDataChange(snapshot)<br/>getValue(GameRoom.class)
        X->>X: showSelectedRoom()<br/>showGameStatus()<br/>ממתין להצטרפות והלוח מושבת
    and עדכון רשימת החדרים אצל המצטרף
        DB-->>O: gamesListener<br/>onDataChange(snapshot)
        O->>O: עדכון הרשימות והתצוגה<br/>rooms, roomIds<br/>roomLabels, Spinner
    end
    O->>O: openSelectedGame()<br/>playerO.isEmpty()
    O->>O: child(roomId).child("playerO")<br/>.setValue(currentUid)
    O->>DB: סנכרון מזהה המצטרף<br/>playerO
    par פתיחת החדר אצל המצטרף לאחר האישור
        DB-->>O: addOnSuccessListener
        O->>O: openRoom(roomId)<br/>addValueEventListener(...)
        O->>O: onDataChange(snapshot)<br/>getValue(GameRoom.class)
        O->>O: showSelectedRoom()<br/>showGameStatus()<br/>localPlayer = O<br/>ממתין לתורו
    and עדכון החדר אצל היוצר
        DB-->>X: selectedRoomListener<br/>onDataChange(snapshot)
        X->>X: getValue(GameRoom.class)<br/>showSelectedRoom()<br/>showGameStatus()
        X->>X: localPlayer = X<br/>תור האיקס והלוח מופעל
    end
```

</div>

הבלוקים המקבילים מציגים תהליכים שאינם מחכים זה לזה: אין הבטחה אם X יציג את ההצטרפות לפני או אחרי ש־O יקבל אישור כתיבה. לעומת זאת, בתוך כל מכשיר `openRoom` של יוצר החדר ושל המצטרף החדש נקראת בקוד רק מתוך `addOnSuccessListener`. שחקן שחוזר לחדר וצופה בחדר פעיל נכנסים דרך `openRoom` ללא כתיבת `playerO`.

מיד אחרי חיבור `selectedRoomListener` מתקבלת גם תמונת המצב ההתחלתית הזמינה ללקוח; אין צורך להמתין למהלך הבא. המאזין מקבל את **כל החדר**, גם כאשר רק `playerO` או `moves` השתנו. לכן שחקן חוזר יכול לבנות את הלוח גם אם לא ראה את האירועים הקודמים. [קריאה באמצעות מאזין קבוע](https://firebase.google.com/docs/database/android/read-and-write#read_data_with_persistent_listeners).

### ב. X לוחץ, O מקבל — ואז הכיוון מתהפך

כעת שני המכשירים כבר מאזינים לאותו חדר, הלוח ריק והתור הוא X. שתי הכתיבות בדוגמה מתקבלות בשרת.
...
sequenceDiagram
    ...
    participant DB as Firebase RTDB<br/>Server
    participant O as Device O<br/>Activity + SDK
    X->>X: onCellClick(view)<br/>בדיקת תפקיד, תור וחוקיות<br/>model.isLegal(row, col)
    X->>X: setBoardEnabled(false)<br/>child("moves")<br/>.setValue("0,0,X")
    X->>X: אירוע מקומי<br/>selectedRoomListener<br/>onDataChange(snapshot)
    X->>X: getValue(GameRoom.class)<br/>showSelectedRoom()<br/>showGameStatus()
    Note over X: האיקס כבר מצויר<br/>התור כעת לעיגול<br/>טרם התקבל אישור כתיבה
    X->>DB: סנכרון המהלך<br/>moves = "0,0,X"
    par הכתיבה מסתיימת אצל השולח
        DB-->>X: הכתיבה אושרה<br/>בקוד אין פעולת הצלחה למהלך
    and השינוי מגיע לשחקן השני
        DB-->>O: selectedRoomListener<br/>onDataChange(snapshot)
        O->>O: getValue(GameRoom.class)<br/>showSelectedRoom()<br/>showGameStatus()
        Note over O: האיקס מצויר ותור העיגול<br/>הלוח מופעל
    end
    O->>O: onCellClick(view)<br/>צירוף המהלך לרצף הקודם
    O->>O: setBoardEnabled(false)<br/>child("moves")<br/>.setValue(updatedMoves)
    O->>O: אירוע מקומי<br/>selectedRoomListener<br/>onDataChange(snapshot)
    O->>O: getValue(GameRoom.class)<br/>showSelectedRoom()<br/>showGameStatus()
    Note over O: האיקס והעיגול מצוירים<br/>תור האיקס<br/>עדיין אין כאן אישור מהשרת
    O->>DB: סנכרון רצף המהלכים המלא
    par הכתיבה מסתיימת אצל השולח
        DB-->>O: הכתיבה אושרה
    and השינוי מגיע לשחקן השני
        DB-->>X: selectedRoomListener<br/>onDataChange(snapshot)
        X->>X: getValue(GameRoom.class)<br/>showSelectedRoom()<br/>showGameStatus()
        Note over X: שני הסימנים מצוירים<br/>תור האיקס<br/>הלוח מופעל
    end
```

</div>

הציור המהיר במכשיר שכתב נקרא **עדכון אופטימי**: ה־SDK מפעיל אירוע מקומי לפני אישור השרת. אישור הכתיבה ועדכון המכשיר השני הם שני דברים שונים; אישור אינו אומר שהמכשיר השני כבר צייר. אין להסתמך גם על אירוע `onDataChange` נוסף רק בגלל שאותה כתיבה אושרה. [אירועים מקומיים ועבודה ללא חיבור](https://firebase.google.com/docs/database/android/read-and-write#work_with_data_offline).

### ג. אותה שרשרת ציור רצה אצל כולם

| המתודה או הפעולה | מה היא עושה בכל מכשיר |
|:---|---:|
| `selectedRoomListener.onDataChange` | מקבלת snapshot של החדר ומציבה אובייקט חדש ב־`selectedRoom` דרך `getValue(GameRoom.class)` |
| `showSelectedRoom` | מציגה את השם, קוראת ל־`model.resetGame()` ול־`resetBoard()`, ואז מפרקת את כל `moves` |
| `model.setMove` ו־`buttonFor(...).setText(...)` | משחזרים כל מהלך גם בלוח הלוגי וגם בכפתור המתאים |
| `model.changePlayer` | מחליפה תור אחרי כל מהלך משוחזר; כך מחושב התור הבא מן הרצף |
| ההשוואה ל־`currentUid` | קובעת לכל מכשיר את `localPlayer`: X, O או מחרוזת ריקה לצופה |
| `showGameStatus` ו־`setBoardEnabled` | מציגות את המצב ומאפשרות לחיצה רק לשחקן שתורו הגיע |

הצופה משתמש באותו מאזין ובאותה שרשרת ציור. הוא מקבל גם את המהלכים שכבר נשמרו וגם שינויים בהמשך. `localPlayer` שלו ריק, ולכן `showGameStatus` משבית את הכפתורים.

### ד. ארבעה אירועים שקל לבלבל

| האירוע | מה מותר להסיק ממנו |
|:---|---:|
| `onDataChange` | המצב הזמין ללקוח השתנה, או שהמאזין קיבל מצב התחלתי. אירוע מקומי אינו אישור שרת |
| `addOnSuccessListener` של `setValue` | הכתיבה הושלמה במסד. בשיעור משתמשים בזה לפתיחת חדר אחרי יצירה או הצטרפות |
| `addOnFailureListener` של `setValue` | פעולת הכתיבה נכשלה. במהלכים הקוד קורא ל־`showGameStatus()` ומציג הודעה |
| `onCancelled` של `ValueEventListener` | ההאזנה נכשלה או בוטלה מצד השרת, למשל עקב הרשאות קריאה; זה אינו ה־callback לכישלון של כל כתיבה |

ההפרדה בין השלמת כתיבה להאזנה מופיעה ב־[תיעוד אישור הכתיבה](https://firebase.google.com/docs/database/android/read-and-write#add_a_completion_callback) וב־[תיעוד `ValueEventListener`](https://firebase.google.com/docs/reference/android/com/google/firebase/database/ValueEventListener).

<div markdown="1" class="box-warning">

התרשימים מתארים מסלול מוצלח, והם אינם מוסיפים לקוד מנגנוני הבטחה:

- בניתוק זמני הכתיבה יכולה להמתין לסנכרון בעוד המכשיר שכתב כבר מציג עדכון מקומי. ניתוק אינו בהכרח כישלון כתיבה מיידי, ואין בשיעור חיווי ייעודי לכתיבה ממתינה.
- שתי כתיבות מקבילות עלולות לדרוס זו את זו. השבתת הכפתורים, ה־getters וה־setters אינם תחליף ל־transaction או לחוקי RTDB.
- `onDestroy()` מסירה את שני המאזינים באמצעות `removeEventListener`. הקוד אינו מסיר אותם רק כי ה־Activity הפסיקה להיות גלויה; הסרת מאזין גם אינה מוחקת את החדר או מבטלת כתיבה שכבר נשלחה.
...
- [SQL](/android/sqlite/01.requery-student)
- [Notifications and animations](/android/CollectCircles/01.collect-circles-drawing)
