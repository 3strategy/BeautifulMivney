...
[Back to 018b: Firebase, RTDB and Authentication setup](/android/projectSteps/018b.FirebaseProjectRtdbAuthSetup){: data-sequence-nav="prev"}
...
## Step 0 - Firebase Gradle wiring

{: .box-note}
The Firebase Assistant in `018b` may already have added an alias containing `gms`. In these diffs, remove only existing old lines and add only missing entries. Red spellings are alternatives; adapt them to your actual alias. Preserve your existing Google Services version, and delete its old version key only if no other entry uses it. Sync after completing all changes.

### 0.1 Gradle Scripts > libs.versions.toml
...
### 0.2 Gradle Scripts > build.gradle.kts (Module :app)
...
{: .box-warning}
**Occasionally, also open Gradle Scripts > build.gradle.kts (Project: …).** If the old alias appears there, replace it as shown below; otherwise leave that file unchanged:
...
Run **Gradle Sync**. The BoM supplies Auth and Database versions, so those libraries have no separate version in TOML. Keep the matching `app/google-services.json` in place.
...
- [018d Google OAuth login + SHA1 setup (Android + Firebase)]({{ '/android/projectSteps/018d.GoogleOAuthLoginAndSHA1' | relative_url }}){: data-sequence-nav="next"}
