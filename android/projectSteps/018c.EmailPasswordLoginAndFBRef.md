...
[Back to 018b: Firebase, RTDB and Authentication setup](/android/projectSteps/018b.FirebaseProjectRtdbAuthSetup)

{: .box-note}
This lesson replaces the fake delayed navigation from `018a` with a real Firebase Email/Password login flow.

{: .box-warning}
This lesson also has a [version here](/android/projectSteps/018c.EmailPasswordLoginAndFBRefHebrewBindings), for those who switched to using bindings (by implementing lesson [019a,b,c](/android/projectSteps/019a.BindingInsteadOfFindByID)).

<div markdown="1" class="hebrew box-warning rtl-notice">

משתמשים כבר ב- Binding או מעדיפים עברית? פתחו את [גרסת עברית + View Binding של  018c]({{ '/android/projectSteps/018c.EmailPasswordLoginAndFBRefHebrewBindings' | relative_url }}).
...
First, register both the Google Services plugin and the Firebase libraries in the version catalog. The three TOML sections have different jobs:

- `[versions]` stores reusable version numbers.
- `[libraries]` declares dependency artifacts used inside `dependencies { ... }`.
- `[plugins]` declares Gradle plugins used inside `plugins { ... }`.

Add the Google Services and Firebase BoM versions under `[versions]`:
...
Add the Firebase dependency artifacts under `[libraries]`:
...
Finally, add only the Google Services Gradle plugin under `[plugins]`:
...
{: .box-warning}
Do not put `firebase-bom`, `firebase-auth`, or `firebase-database` under `[plugins]`. They are libraries, not Gradle plugins. Entries under `[plugins]` use an `id`; dependency coordinates using `group` and `name` belong under `[libraries]`.

These catalog keys generate the following Gradle accessors:

- `google-services` becomes `libs.plugins.google.services`.
- `firebase-bom` becomes `libs.firebase.bom`.
- `firebase-auth` becomes `libs.firebase.auth`.
- `firebase-database` becomes `libs.firebase.database`.

### 0.2 `app/build.gradle.kts` (module)

Only after updating `libs.versions.toml`, add the Google Services plugin alias to the module's `plugins` block:
...
Then add the Firebase dependencies:
...
{: .box-note}
`firebase-auth` and `firebase-database` intentionally have no version in `[libraries]`. The Firebase BoM supplies compatible versions for them through `platform(libs.firebase.bom)`.
Make sure the matching `app/google-services.json` is present before syncing or building.
...
### Recommended: add a simple drawer-menu logout ("Disconnect") {#idAddLogout}
...
- [018d Google OAuth login + SHA1 setup (Android + Firebase)]({{ '/android/projectSteps/018d.GoogleOAuthLoginAndSHA1' | relative_url }})
- [למד אותי: קישור ל-GEM](https://gemini.google.com/gem/1Q_lIRv8lGndHJSXSSc2PmQakGc47EiLL?usp=sharing)


{: .box-note}
**Optional design detour:** To give `TicTacMenu` the blue/orange palette, blue drawer toolbar, and clear square game cells used by the sibling sandbox, follow [the small theming-alignment tutorial]({{ '/android/projectSteps/020tictacmenu-theme-alignment' | relative_url }}). This visual polish is optional and is not required to complete `018c` or continue to `018d`.
