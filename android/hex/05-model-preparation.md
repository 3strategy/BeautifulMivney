...
עבדו לפי סדר התלות: הוסיפו את התלות והנכסים, הרחיבו את מחלקת החוקים וצרו את `ValueModel`; בסוף העתיקו את קובץ ה־Java המסופק. במעבר על diff אל תקלידו את סמלי `+` ו־`-` עצמם. פתחו כל תיבת קוד של שינוי מלא וקראו עד סוף התוכן; אם הקוד ממשיך מתחת למסך, גללו בעמוד.
...
בצעו Sync ובנו את הפרויקט (`assembleDebug`), ואז הפעילו את האפליקציה. שחקו כמה מהלכים במצב המקומי ולחצו Restart: המשחק הקיים צריך להמשיך לעבוד. השינוי בפרק הזה הוא בקוד שמתחת למסך, ולכן עדיין לא מופיע יריב מחשב.
