...
הפונקציה הראשונה מבצעת $$n^2$$ חיבורי איברים ולכן יש לה סיבוכיות **ריבועית**, כלומר, $$O(n²)$$. הפונקציה השנייה עוברת על מערך חד-ממדי ולכן היא בסיבוכיות ליניארית, כלומר, $$O(n)$$


## את הסיבוכיות יש לרשום תמיד עם n, כלומר יש לרשום תמיד $$O(n), O(n²), O(\log n)$$, אבל חשוב להגדיר מהו n
ברישום סיבוכיות תמיד מתבטאים במונחי n בלי קשר לשמות המשתנים.
כך למשל עבור פונקציה סוכמת `int Sum(arr[])` הסיבוכיות היא $$O(n)$$ ו-$$n$$ הוא אורך המערך `arr`
...
נתון מערך **לא ממויין** של מספרים שלמים. עליכם לבדוק האם קיים תת-רצף רציף (subarray) אשר סכום האיברים בו שווה למספר נתון $s$.
...
* כתבו פונקציה ב-C# שמקבלת `int[] arr`, מספר `int target` ומחזירה `bool` האם קיים תת-רצף רציף שסכומו שווה target.
...
כתבו פונקציה ב-C# שמקבלת מערך ומחזירה את מספר הרצפים.
