...
| מחזיר את הפריט בתחילת התור מבלי להסיר | `()T Head` |
...
{: .subq}
א. בתכנות "אמיתי" אל תשתמשו בהזזה של כל האיברים ב־`Remove`; השתמשו במערך מעגלי או רשימה מקושרת כדי להשיג סיבוכיות O(1).  
{: .subq}
ב. ודאו שאתם בודקים אם התור ריק לפני קריאה ל‑`Remove` או `Head`.  
{: .subq}
